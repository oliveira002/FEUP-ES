...
import 'package:uni/view/Widgets/subject_without_gesture.dart';
...
class OverviewPageView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: <Widget>[
      ListView(
        ...
        shrinkWrap: true,
        children: <Widget>[
          PageTitle(name: 'Horário Sobreposto'),
          TabBar(
  ...
  List<Widget> createScheduleRows(lectures, BuildContext context) {
    ...
    for (int i = 0; i < lectures.length; i++) {
      ...
      scheduleContent.add(ScheduleWithoutSlot(
  ...
  Widget createScheduleByDay(BuildContext context, int day) {
    return RequestDependentWidgetBuilder(
      ...
      onNullContent:
      Center(child: Text('O Gupo não possui aulas à ' + daysOfTheWeek[day] + '.')),
      index: day,
    );
  }
}